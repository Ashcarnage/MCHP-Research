...
from keras.layers import Input, Conv2D, MaxPooling2D, concatenate, Conv2DTranspose, BatchNormalization, Dropout, Lambda,Conv3D
...
def simple_unet_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS, num_classes):
    ...
    c3 = Conv2D(64, (3, 3), activation='relu', kernel_initializer=kernel_initializer, padding='same')(s)
    # c3 = Dropout(0.2)(c3)
    # c3 = Dropout(0.1)(c3)
    ...
    outputs = Conv2D(IMG_CHANNELS, (1, 1), activation='sigmoid')(c7)
    
    model = Model(inputs=[inputs], outputs=[outputs])
    return model

# Test the model
model = simple_unet_model(256, 256, 20, 4)
